...
export default function Nav() {
    ...
    return (
        <nav
            className="fixed bottom-0 w-full py-6 px-1 overflow-x-auto flex flex-row items-center cs-nav-2:justify-center gap-16 bg-nav-bg lg:flex-col lg:top-0 lg:left-0 lg:w-fit lg:min-w-40 lg:h-fit lg:px-8 lg:items-start lg:mt-8 lg:bg-transparent z-50"
        >

            {
                allLinks.map((link, index) => (
                    <NavigationLink key={link['linkText']} linkDetails={link} index={index} />
                ))
            }
